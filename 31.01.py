'''class DEtails:
    def __init__(self):
        self.seats={'A':20,'B':19,'C':18,'D':,17,'E'=16,'F'=15,'G'=14,'H'=13,'I':12.'J':11,'K':10,'L':9,'M',:8,'N':7,'O':6,'P':5,'Q':4,'R':3,'S':2,'T':1,'U':0,'V':0,'W':0,'X':0,'Y':0,'Z':0}
        self.prices={
    ...
    def display_seats(self):
        print("Aviable Seats Are:")
        for row,cols in self.seats.items():
            print(f"{row}:{cols}")
    def book_seats(self):
        name=input("Enter the Name:")
        phone_number=int(input("Enter your Phone Numbers:"))
        date_of_birth=input("Enter you date f birht:")
        num_seats=int(input("Enter the Number of seats:"))
        self.display_seats()
        seat_selection=input("Select Your seats (e.g:,aA2-A6):")
